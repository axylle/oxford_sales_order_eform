...
angular.module('dynform', [])
  .directive('dynamicForm', ['$q', '$parse', '$http', '$templateCache', '$compile', '$document', '$timeout', function ($q, $parse, $http, $templateCache, $compile, $document, $timeout) {
    var supported = {
        ...
        'xcanvas': {element: 'div', editable: false, textBased: false},
        ...
        'canvas': {element: 'canvas', editable: false, textBased: false},
            ...
            var buildFields = function (field, id) {
              if (!angular.isDefined(supported[field.type]) || supported[field.type] === false) {
                ...
                if (angular.isDefined(supported[field.type].editable) && supported[field.type].editable) {
                  ...
                  newElement.attr('caption', ((angular.isDefined(field.caption)) ? field.caption : field.label));
                    
                  if (angular.isDefined(field.readonly)) {newElement.attr('readonly', field.readonly);}
                ...
                if (field.type === 'number' || field.type === 'range') {
                  ...
                  if (angular.isDefined(field.source)) {newElement.attr('src', field.source);}
									newElement.attr('id', field.id);
									newElement.attr('drawing', 'drawing');
									newElement.attr('style', field.style);
                }
                else if (field.type === 'xcanvas') {
									if (angular.isDefined(field.id)) newElement.attr('id', field.id);
									if (angular.isDefined(field.classname)) newElement.attr('class', field.classname);
                ...
                if (field.type !== "radio") {
                  ...
                  if (field.type !== "checklist") {
										...
										if (angular.isDefined(field.disabled)) {newElement.attr('disabled', field.disabled);}
										if (angular.isDefined(field.callback)) {
											//  Some input types need listeners on click...
											if (["button", "fieldset", "image", "legend", "reset", "submit", "xcanvas"].indexOf(field.type) > -1) {
												cbAtt = 'ng-click';
											}
											//  ...the rest on change.
											else {
                ...
                if (angular.isDefined(field.label)) {
                  ...
                  //  Everything else should be wrapped in a label tag.
                    ...
                    var lbl = '<label></label>';
                    if (angular.isDefined(field.required)) lbl = '<label>*</label>';
                    newElement = newElement.wrap(lbl).parent();
                ...
                if (angular.isDefined(field.attributes)) {
                  ...
                  angular.forEach(field.attributes, function (val, attr) {
                    if (attr==="style") req_style=val;
                    newElement.attr(attr, val);
                  });
                  if (angular.isDefined(field.required)) {
                    newElement.removeAttr("style");
                    newElement.attr("style", req_style+";color:red;");
                  }
                }
  ...
  .directive('input', ['$parse', function ($parse) {
	...
	.directive("drawing", [function(){
		return {
			/*template: '<span style="padding:4px;background-color:#D9D7D8" class="glyphicon glyphicon-remove" id="obj_sign1_trash" onClick="$sign.clear_signature(\'obj_sign1\',\'obj_sign1_xy\')"></span>',
			restrict: 'E',
      transclude: true,
      replace:true,
      scope:true,*/
			link: function(scope, element){
				/*var newDirective = null;
				newDirective = angular.element('<span>asdf</span>');
